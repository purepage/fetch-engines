...
Sends a POST request while preserving cookies and stealth behavior. Useful for login flows or form submissions that need a browser-like session. When passing a plain string as the body, provide an appropriate `Content-Type` header so the server interprets it correctly.
