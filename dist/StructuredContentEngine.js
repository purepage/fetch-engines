...
export class StructuredContentEngine {
    ...
    async fetchStructuredContent(url, schema, options) {
        ...
        if (!apiKey) {
            throw new Error("API key is required for structured content extraction. Provide it via apiConfig.apiKey or set OPENAI_API_KEY or OPENROUTER_API_KEY environment variables");
        }
