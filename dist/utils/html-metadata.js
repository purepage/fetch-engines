...
function maskNonMarkup(html) {
    ...
    const blank = (start, end) => masked.fill(" ", start, end);
    for (let start = 0; start < html.length; start++) {
        ...
        if (!/[A-Za-z]/.test(html[start + 1] ?? ""))
            continue;
        let cursor = start + 1;
        while (TAG_NAME_CHARACTER.test(html[cursor] ?? ""))
            cursor++;
        for (; cursor < html.length && html[cursor] !== ">"; cursor++) {
            if (html[cursor] !== "=")
                continue;
            let valueStart = cursor + 1;
            while (WHITESPACE.test(html[valueStart] ?? ""))
                valueStart++;
...
function isInTitleIgnoringContext(element) {
    for (let parent = element.parentNode; parent; parent = parent.parentNode) {
        if (TITLE_IGNORING_ANCESTORS.has(parent.rawTagName?.toLowerCase()))
            return true;
    }
    return false;
}
function isInsideSlashRawTextElement(title, elements, html, maskedHtml) {
    return elements.some((element) => {
        const tagName = element.rawTagName.toLowerCase();
        const openingEnd = maskedHtml.indexOf(">", element.range[0]) + 1;
        if (!RAW_TEXT_TAGS.has(tagName) || !/\/\s*>$/.test(maskedHtml.slice(element.range[0], openingEnd)))
            return false;
...
export function extractHtmlTitle(html) {
    ...
    for (const title of document.querySelectorAll("title")) {
        if (isInTitleIgnoringContext(title) || isInsideSlashRawTextElement(title, elements, html, maskedHtml))
            continue;
        const closingTag = /<\/title[\t\n\f\r ]*>/gi;
        closingTag.lastIndex = title.range[0];
        if (!closingTag.exec(maskedHtml))
            continue;
