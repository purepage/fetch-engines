...
const createMockContext = (mockBrowser: Browser): BrowserContext => {
...
describe("PlaywrightBrowserPool", () => {
  ...
  describe("Initialization", { timeout: 60000 }, () => {
    ...
    it("should start health checks after initialization", async () => {
      ...
      const healthCheckSpy = vi.spyOn(pool as any, "healthCheck");
  ...
  describe("Page Acquisition", () => {
    ...
    it("should fail acquisition if pool is full and all instances are at max pages", async () => {
      ...
      await expect(pool.acquirePage()).rejects.toThrow(
        /Failed to acquire Playwright page.*pool may be unhealthy or overloaded/i
      );
    ...
    it("should fail acquisition if pool is unhealthy or becomes unhealthy", async () => {
      ...
      const browserInstance = (pool as any).pool[0] as {
        isHealthy: boolean;
        pages: Set<Page>;
      };
  ...
  describe("Health Checks", { timeout: 60000 }, () => {
    ...
    it("should remove an instance due to idle timeout (if pool size > 1)", async () => {
      ...
      (pool as any).maxIdleTime = idleTimeout;
    ...
    it("should remove an instance due to browser disconnect event", async () => {
      ...
      (mockBrowser as any).emit("disconnected");
    ...
    it("should keep healthy instances during health check", async () => {
      ...
      const instance1 = (pool as any).pool[0] as { browser: Browser };
      const instance2 = (pool as any).pool[1] as { browser: Browser };
    ...
    it("should replenish instances after removal to maintain pool size", async () => {
      ...
      const instanceToRemove = (pool as any).pool[0];
      await (pool as any).closeAndRemoveInstance(instanceToRemove); // Use internal method for testing
