...
export class PlaywrightEngine implements IEngine {
  ...
  async postHTML(
    ...
    const result = await this.queue.add(() => this.postWithPlaywright(url, body, headers, convertToMarkdown));
    return result;
  }

  private async postWithPlaywright(
    url: string,
    body: string | URLSearchParams | FormData,
    ...
    try {
      ...
      const serialisedBody = isFormData ? Array.from(body.entries()) : isURLSearchParams ? body.toString() : body;

      const fetchResult = await page.evaluate(
        async ({ url, body, headers, isFormData, isURLSearchParams }) => {
          let payload: any = body;
          if (isFormData) {
            const form = new FormData();
            for (const [k, v] of body as any) {
              form.append(k as string, v as string);
            }
            payload = form;
          }
          const response = await fetch(url, {
            method: "POST",
            headers,
            body: isURLSearchParams ? (body as string) : payload,
            credentials: "include",
          });
        ...
        { url, body: serialisedBody, headers, isFormData, isURLSearchParams }
      );
      ...
      let finalContent = fetchResult.text;
      let finalContentType: "html" | "markdown" = "html";

      if (convertToMarkdown) {
        try {
          const converter = new MarkdownConverter();
          finalContent = converter.convert(fetchResult.text);
          finalContentType = "markdown";
        } catch (conversionError: any) {
      ...
      const titleMatch = fetchResult.text.match(/<title[^>]*>([^<]+)<\/title>/i);
   ...
   */
  private checkContentCache(cacheKey: string): ContentFetchResult | null {
   ...
   */
  private addContentToCache(cacheKey: string, result: ContentFetchResult): void {
    if (this.config.cacheTTL <= 0) return; // Caching disabled

    // Convert to HTMLFetchResult format for storage
    const htmlResult: HTMLFetchResult = {
   ...
   */
  private async _fetchContentRecursive(
    url: string,
    currentConfig: Required<
      ContentFetchOptions & {
        retryDelay: number;
   ...
   */
  private async _attemptContentHttpFallback(
    url: string,
    currentConfig: Required<ContentFetchOptions & { useHttpFallback: boolean; headers?: Record<string, string> }>
   ...
   */
  private async fetchContentWithPlaywright(
    url: string,
    pool: PlaywrightBrowserPool,
    fastMode: boolean,
    headers?: Record<string, string>
