...
const SOFT_BLOCK_TITLE_REGEX =
  /just a moment|attention required|access denied|please wait|one more step|checking your browser|security check|you have been blocked|blocked by|are you a robot/i;

const SOFT_BLOCK_BODY_REGEX =
  /checking your browser|verify you.{0,10}(?:are |'re )?(?:not a )?(?:ro)?bot|verify you.{0,10}human|please complete the security check|cf-challenge|captcha-container|hcaptcha|recaptcha|cf-turnstile|enable (?:javascript|cookies) to (?:continue|access|view)|automated (?:access|request)|bot detect|suspicious activity|unusual traffic|too many requests|rate limit exceeded|we need to verify/i;
 ...
 * actual page content.
 */
export function isSoftBlockPage(html: string): boolean {
  const visibleText = stripHtmlToVisibleText(html);
  ...
  if (SOFT_BLOCK_TITLE_REGEX.test(title)) return true;
