...
export class MarkdownConverter {
  ...
  public convert(html: string, options: ConversionOptions = {}): string {
    ...
    let markdown: string;
    try {
      markdown = kreuzbergConvert(preprocessedHtml, { headingStyle: "Atx" as never });
    } catch (error: unknown) {
  ...
  private removeSvgImageRefs(root: NHPHTMLElement): void {
    root.querySelectorAll("img[src]").forEach((el) => {
      const src = (el as NHPHTMLElement).getAttribute("src") || "";
      if (src.toLowerCase().includes(".svg")) el.remove();
    });
  }
  ...
  private ensurePrimaryHeading(_root: NHPHTMLElement, content: NHPHTMLElement | NHPNode, providedTitle?: string): void {
    if (!(content instanceof NHPHTMLElement)) return;
    ...
    if (titleNorm) {
      const h1 = parse(`<h1>${providedTitle ?? ""}</h1>`).firstChild as NHPHTMLElement;
      content.prepend(h1);
      return;
    }
  ...
  private hasClass(cls: string, exact: string): boolean {
    return cls.split(/\s+/).some((token) => token === exact);
  }

  private hasClassSubstring(cls: string, sub: string): boolean {
  ...
  private elementMatchesMainContent(el: NHPHTMLElement): boolean {
    ...
    if (
      this.hasClassSubstring(cls, "article-body") ||
      ...
      return true;
    if (id.includes("article-body") || id.includes("main-content")) return true;
    return false;
  }

  /** Check if element matches boilerplate selectors (node-html-parser has no matches()). */
  private elementMatchesBoilerplate(el: NHPHTMLElement): boolean {
    const tag = el.tagName?.toLowerCase() || "";
    const role = (el.getAttribute?.("role") || "").toLowerCase();
    const cls = (el.getAttribute?.("class") || "").toLowerCase();
    ...
    if (
      this.hasClassSubstring(cls, "sidebar") ||
  ...
  private _calculateElementScore(element: NHPHTMLElement, currentMaxScore: number): number {
    ...
    if (this.elementMatchesBoilerplate(element)) score *= 0.2;
