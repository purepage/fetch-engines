...
function findTitle(node: Node, ignoredContext: boolean): string | null | undefined {
  if (!(node instanceof HTMLElement)) return undefined;

  const tagName = node.rawTagName?.toLowerCase();
  if (tagName === "plaintext") return null;

  const titleIgnoringContext = ignoredContext || (tagName ? TITLE_IGNORING_CONTEXTS.has(tagName) : false);
  if (tagName === "title" && !ignoredContext) return node.textContent.trim() || null;
